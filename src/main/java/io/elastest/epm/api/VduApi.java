...
public interface VduApi {
    ...
    @ApiOperation(value = "Terminates a VDU.", notes = "Terminates the VDU that matches with a given ID.", response = String.class, tags={ "VDU", })
    @ApiResponses(value = {
    ...
    @RequestMapping(value = "/vdu/{id}",
        produces = { "*/*" }, 
        consumes = { "application/json" },
        method = RequestMethod.DELETE)
    ResponseEntity<String> deleteVdu(@ApiParam(value = "ID of VDU", required = true) @PathVariable("id") String id) throws TerminationException, NotFoundException;


    @ApiOperation(value = "Allocates resources in the target cloud.", notes = "Allocates resources defined as a VDU in the cloud to be deployed in the target cloud environment. It instantiates computing resources, deploys artifacts on them and manages the their lifecycle", response = VDU.class, tags={ "VDU", })
    @ApiResponses(value = {
    ...
    @ApiOperation(value = "Returns a VDU.", notes = "Returns the VDU with the given ID. Returns all its details.", response = VDU.class, tags={ "VDU", })
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "Successful operation", response = VDU.class),
        @ApiResponse(code = 401, message = "Unauthorized", response = VDU.class),
        @ApiResponse(code = 403, message = "Forbidden", response = VDU.class),
    ...
    ResponseEntity<VDU> updateVdu(@ApiParam(value = "ID of VDU", required = true) @PathVariable("id") String id, @ApiParam(value = "VDU object that needs to be updated.", required = true) @RequestBody VDU body);
